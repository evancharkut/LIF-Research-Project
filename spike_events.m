...
if nargin < 2
    error('spike_events:notEnoughInputs', ...
          ['SPIKE_EVENTS needs a raster and a duration:\n' ...
           '    ev = spike_events(spikes_by_trial, t_end)\n' ...
           'It is a function, not a script -- typing its name alone calls ' ...
           'it with no arguments. Run LIF_JITTER_RELIABILITY for a worked ' ...
           'example.']);
end
...
if opts.t_start >= t_end
    error('spike_events:startAfterEnd', ...
          ['opts.t_start (%g ms) is not before t_end (%g ms), so every ' ...
           'spike would be discarded. The default t_start of 1000 ms ' ...
           'assumes a run of several seconds; lower it for short runs.'], ...
          opts.t_start, t_end);
end
